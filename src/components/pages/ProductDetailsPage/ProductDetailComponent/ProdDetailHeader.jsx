    ...
    <>
      ...
      <div id="headerBg" className="flex flex-row-reverse pb-4 max-w-full max-h-full">
        <div id="sideDesc" className="ml-4 min-w-60 w-screen max-w-full max-h-full">
          ...
          <div className="reviews flex text-xs">
            ...
            <div className="reviewsScore ml-4 text-buttonColor flex flex-col">
              ...
              <span className={`${isRecommendLength < 10 ? "text-red-500" : "text-blue-500"}`}>{isRecommendLength < 5 ? "Negative" : "Positive"}</span>
              <span className="text-fontColor leading-8">{formatDate(product.release_date)}</span>
        ...
        <div id="mainVideo" className="w-full h-full">
          {mainContent.type === "video" ? <video controls src={mainContent.src} type="video"></video> : <img src={mainContent.src} alt="Main Preview" />}
            ...
            <Swiper modules={[Navigation, Pagination, Scrollbar, A11y]} slidesPerView={5} className="lg:w-prodSliderSize lg:h-auto" scrollbar={{ draggable: true }}>
              {product?.ScrollThumbnails?.map((src, index) => (
                <SwiperSlide key={index}>
                  <img src={src.img} alt={`Screenshot ${index + 1}`} className="thumbnail" onClick={() => handleThumbnailClick(src.img, "image")} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
